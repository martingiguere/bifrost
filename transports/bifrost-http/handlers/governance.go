...
func (h *GovernanceHandler) getComplexityAnalyzerConfig(ctx *fasthttp.RequestCtx) {
	...
	raw, err := configstore.GetComplexityAnalyzerConfigRaw(ctx, h.configStore)
	if err != nil {
	...
	if err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("stored complexity analyzer config is invalid: %v", err))
		return
	}
	SendJSON(ctx, cfg)
}
...
func (h *GovernanceHandler) updateComplexityAnalyzerConfig(ctx *fasthttp.RequestCtx) {
	...
	if err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("failed to marshal complexity analyzer config: %v", err))
		return
	}

	if err := configstore.UpdateComplexityAnalyzerConfigRaw(ctx, h.configStore, normalizedRaw); err != nil {
