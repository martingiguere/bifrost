...
func TestSQLite_ComplexityAnalyzerConfig_FileSeeded(t *testing.T) {
	...
	configData := makeConfigDataWithProvidersAndDir(nil, tempDir)
	configData.Governance = &configstore.GovernanceConfig{
		ComplexityAnalyzerConfig: mustMarshalComplexityAnalyzerConfig(t, analyzerConfig),
	}
	createConfigFile(t, tempDir, configData)
	...
	require.NotEmpty(t, config.GovernanceConfig.ComplexityAnalyzerConfig)
	loadedConfig, err := complexity.DecodeAndValidate(config.GovernanceConfig.ComplexityAnalyzerConfig)
	require.NoError(t, err)
...
func TestSQLite_ComplexityAnalyzerConfig_FileWinsOnRestartWhenPresent(t *testing.T) {
	...
	configData := makeConfigDataWithProvidersAndDir(nil, tempDir)
	configData.Governance = &configstore.GovernanceConfig{
		ComplexityAnalyzerConfig: mustMarshalComplexityAnalyzerConfig(t, seedCfg),
	}
	createConfigFile(t, tempDir, configData)
	...
	require.NotEmpty(t, config2.GovernanceConfig.ComplexityAnalyzerConfig)
	reloadedConfig, err := complexity.DecodeAndValidate(config2.GovernanceConfig.ComplexityAnalyzerConfig)
	require.NoError(t, err)
