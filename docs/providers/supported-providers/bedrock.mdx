...
Bifrost signs every Bedrock request with AWS Signature Version 4 (SigV4). Four authentication methods are supported — choose the one that matches your deployment environment.
...
Uses the IAM role attached to your environment (IRSA in EKS, ECS task role, EC2 instance profile). If no static credentials are stored — Bifrost auto-detects the identity from the runtime environment.
